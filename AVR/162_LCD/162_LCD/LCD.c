...
static void lcd_enable_pulse(void)
{
	...
	_delay_us(1);                                                           // these delay values come from datasheet (min 450 nano sec)
...
{
		uint8_t addr = (row ? 0x40 : 0x00) + (col & 0x0F);					// If row == 1, set addr1 = 0x40 (start of second LCD line)
		lcd_cmd(0x80 | addr);		// send (80 + addr) as a command.										// If row == 0, set addr1 = 0x00 (start of first LCD line)
														                   // Equivalent to: addr1 = row ? 0x40 : 0x00;
...
{
	while (*s)									// Every C string ends with '\0' (null terminator)
												// This condition checks if the current character *s is '\0'
												// Used to detect end of string during traversal
	{
		lcd_data((uint8_t)*s++);
	}
