    ...
    while(1)
    	{
    	 ...
    	 //I2C send and receive data
    	    I2C_MasterSendData(&I2C1_handle,&reg_add,1,0x76,I2C_ENABLE_SR);
    	    I2C_MasterReceiveData(&I2C1_handle,&chip_id, 1, 0x76,I2C_DISABLE_SR);


    	    char msg[30];

    	    sprintf(msg,"Chip ID = 0x%X\r\n",chip_id);

    	    USART_SendData(&Uart_handle,
    	                   (uint8_t*)msg,
    	                   strlen(msg));
    	   //UART send data to pc
    	   // USART_SendData(&Uart_handle,&chip_id,1);
    	 if (chip_id == 0x58)
    	 {
